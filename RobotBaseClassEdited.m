classdef RobotBaseClassEdited < handle
    %% RobotBaseClassEdited The base robot class from which other UTS robot models should inherit 
...
%% General class for multiDOF robot simulation
        function self = RobotBaseClassEdited()
            ...
            try 
                ...
                self.name = ['RobotBaseClassEdited',datestr(now,'yyyymmddTHHMMSSFFF')];
...
% First and only time to plot the robot
        function h = InitiliseRobotPlot(self)
            ...
            self.workspace = [-2 2 -2 2 -0.5 1.5]; %% SET THE WORKSPACE SIZE HERE!!!
